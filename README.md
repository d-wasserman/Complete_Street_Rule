...
## CityEngine 2025 Street Designer Integration

The **Complete_Lanes** folder introduces a modular, per-lane rule architecture designed for use with CityEngine 2025's **Street Designer**. Each lane type — through lanes, bike lanes, bus lanes, medians, sidewalks, and more — is now a discrete `.cga` rule file that can be assigned individually to lane shapes generated by Street Designer. This enables a more flexible, component-driven workflow compared to the previous monolithic street rule approach.

<!-- TODO: Add GIF demonstrating Complete_Lanes assignment workflow in CityEngine 2025 Street Designer -->
![Street Designer Complete Lanes Demo - GIF Placeholder](/images/CompleteLanes_StreetDesigner_Demo.gif)
...
* **Quick Visualization of Complete Street Features:** The rule can be used to quickly iterate on high level cross-sectional designs for streets through changes to its parameters. The features that can be visualized include bike lanes, bike lane buffers, shared-use lanes, bus lanes, HOV lanes, parking lanes, medians, two-way left turn lanes, and sidewalks featuring trees, street furniture, and other amenities.

* **Modular Complete_Lanes Architecture (CityEngine 2025+):** The `rules/Complete_Lanes/` folder provides individual lane rule files compatible with CityEngine 2025 Street Designer. Each lane type is a self-contained `.cga` file, making it easy to assign, swap, and customize individual lanes within a Street Designer street network. Shared constants, textures, and utility functions are centralized in `Reference.cga`.
